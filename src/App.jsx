...
function App() {
    ...
    const [tasks, setTasks] = useState(() => loadPlan()?.tasks ?? null);
    ...
    function handleMoveTask(id, col) {
    ...
    function handleAddTask(title, description) {
        setTasks((ts) => {
            ...
            return [...ts, { id: nextId, title, description, col: "todo" }];
        });
    }

    function handleEditTask(id, updates) {
    ...
    async function handleRevise(instruction) {
    ...
    function handleApplyRevision() {
        setTasks((ts) => applyRevision(ts, pendingRevision));
